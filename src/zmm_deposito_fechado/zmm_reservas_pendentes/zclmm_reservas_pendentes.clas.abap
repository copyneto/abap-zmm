CLASS zclmm_reservas_pendentes DEFINITION INHERITING FROM cl_abap_behv
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: ty_entity   TYPE zi_mm_reservas_pendentes_app,
           ...
           ty_reported TYPE RESPONSE FOR REPORTED EARLY zi_mm_reservas_pendentes_app.
...
CLASS zclmm_reservas_pendentes IMPLEMENTATION.
...
* ---------------------------------------------------------------------------
  ...
  METHOD build_reported.
    ...
    LOOP AT it_return INTO DATA(ls_return).
      ...
      CREATE DATA  lo_dataref TYPE LINE OF ty_reported-reservas.
...
* ---------------------------------------------------------------------------
      es_reported-reservas[]  = VALUE #( BASE es_reported-reservas[] ( CORRESPONDING #( <fs_cds> ) ) ).
  ...
  METHOD entity_to_table.
    ...
    rt_result = VALUE ty_t_hist( FOR ls_entity IN it_entity (
                            ...
                            storage_location          = ls_entity-StorageLocation
                            plant_dest                = ls_entity-DestinyPlant
                            storage_location_dest     = ls_entity-DestinyStorageLocation
                            batch                     = ls_entity-Batch
*                            freight_order_id          =
                            delivery_document         = |{ ls_entity-Reservation ALPHA = OUT }|
                            process_step              = 'F13'
*                            guid          =
                            status                    = ls_entity-statushistorico
                            origin_unit               = ls_entity-BaseUnit
                            unit                      = ls_entity-Unit
                            order_quantity_unit       = ls_entity-Unit
                            ...
                            origin_plant              = ls_entity-Plant
                            origin_plant_type         = ls_entity-OriginPlantType
                            origin_storage_location   = ls_entity-StorageLocation
...
* 16 - Botão Criar NFe
* ---------------------------------------------------------------------------
*    AUTHORITY-CHECK OBJECT 'ZMM376F13'
