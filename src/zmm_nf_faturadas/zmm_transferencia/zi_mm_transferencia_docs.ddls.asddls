...

{
  ...
  key _NFItem.BR_NotaFiscalItem                                                                as NumeroDocumentoItem,
      ...
      case when (_PurchasingDocument.PurchasingDocumentType = 'UB' or _PurchasingDocument.PurchasingDocumentType = 'NB')
            and (_Ekpa.lifn2 is not initial or _Ekpa.lifn2 is not null)
           then cast( ltrim( _Ekpa.lifn2, '0' ) as werks_d)

           when (_PurchasingDocument.PurchasingDocumentType = 'UB' or _PurchasingDocument.PurchasingDocumentType = 'NB')
            and (_Ekpa.lifn2 is initial or _Ekpa.lifn2 is null )  and _VerifyNF.BR_NFReceiverType = 'B'
            and (_LocalCentroDestino.IssuingOrReceivingPlant is not initial or _LocalCentroDestino.IssuingOrReceivingPlant is not null)
           ...
           then case when length(_EkpoCol.werks) >= 10
           ...
           else cast(' ' as werks_d) end                                                       as LocalNegocioDestino,

      _MatDocItemRef.MaterialDocument                                                          as teste1,
      _EkbeDocRef1.belnr                                                                       as teste2,

      case when _MatDocItemRef.MaterialDocument is initial
             or _MatDocItemRef.MaterialDocument is null
           then  concat( _EkbeDocRef1.belnr, _EkbeDocRef1.gjahr )

           when _MatDocItemRef.ReversedMaterialDocument is initial
             or _MatDocItemRef.ReversedMaterialDocument is null
           then concat( _MatDocItemRef.MaterialDocument, _MatDocItemRef.MaterialDocumentYear )

           else '' end                                                                         as DocRefEntreda1,
      ...
      case when ( _VerifyNF.BR_NFReceiverType = 'B' or _VerifyNF.BR_NFReceiverType ='C' )
            and ( _MatDocItemRef.MaterialDocument is not initial and _MatDocItemRef.MaterialDocument is not null )
           then _MatDocItemRef.DocumentDate
           
           when ( _VerifyNF.BR_NFReceiverType = 'B' or _VerifyNF.BR_NFReceiverType ='C' )
            and ( _Ekbe.budat is not initial and _Ekbe.budat is not null )
           then _Ekbe.budat
      ...
      case when ( _MatDocItemRef.ReversedMaterialDocument is not initial and _MatDocItemRef.ReversedMaterialDocument is not null )
           then 'P'
           when ( _MatDocItemRef.GoodsMovementType = '861' or _MatDocItemRef.GoodsMovementType = '101')
            and _VerifyNF.BR_NFReceiverType = 'C'
           then ''
      ...
      case when NFBrief.HeaderWeightUnit is initial
           then _Mara.gewei
...
group by
  ...
  _MatDocItem.Plant,
  _MatDocItemRef.ReversedMaterialDocument
