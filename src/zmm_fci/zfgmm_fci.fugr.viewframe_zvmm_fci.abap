*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVMM_FCI
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVMM_FCI            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVMM_FCI'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVMM_FCI_EXTRACT
TOTAL = ZVMM_FCI_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVMM_FCI'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVMM_FCI-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVMM_FCI_EXTRACT
TOTAL = ZVMM_FCI_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVMM_FCI-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVMM_FCI'
...
TOTAL = ZVMM_FCI_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVMM_FCI-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVMM_FCI-UPD_FLAG NE SPACE.
