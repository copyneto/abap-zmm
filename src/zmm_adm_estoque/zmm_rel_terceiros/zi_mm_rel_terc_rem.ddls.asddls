...
@EndUserText.label: 'Busca por Remessa'
...
define view entity ZI_MM_REL_TERC_REM
  ...
  //                                                            and MatDoc.InventorySpecialStockType is not initial
    left outer to one join I_MaterialDocumentItem as MatDoc on  MatDoc.MaterialDocument         = Item.BR_NFSourceDocumentNumber
                                                            ...
                                                            and (
                                                               MatDoc.IssgOrRcvgSpclStockInd    = 'O'
                                                               or MatDoc.IssgOrRcvgSpclStockInd = 'M'
                                                               or MatDoc.IssgOrRcvgSpclStockInd is initial
                                                             )
                                                            and MatDoc.Material                 = Item.Material
                                                            and MatDoc.MaterialDocumentItem     = right(
      Item.BR_NFSourceDocumentItem, 4
    )
  //                                                            and MatDoc.InventorySpecialStockType is not initial
