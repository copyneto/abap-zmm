...
*  ENDIF.
  ...
  DELETE ls_doc-xtkomv WHERE kschl = 'RA01' AND kwert = 0.
  DELETE ls_doc-xtkomv WHERE kschl = 'ZDAC' AND kwert = 0.
  ...
  IF lt_komv[] IS NOT INITIAL.
    ...
    SORT ls_doc-xtkomv BY kposn
                          kschl.

    SORT lt_komp BY kposn.
  ENDIF.
...
*        WHERE kunnr EQ t001w-kunnr.
   ...
   AND ls_ekpo-bstyp EQ 'F'.

      SELECT SINGLE *
        FROM adrc
       WHERE addrnumber EQ ls_ekpo-adrn2.

    ELSEIF ls_ekpo-emlif IS NOT INITIAL
       AND ls_ekpo-bstyp EQ 'F'.
...
FORM f_busca_fornecedor USING uv_lifnr
  ...
  SELECT SINGLE adrnr stcd1 stcd3
    FROM lfa1
...
FORM f_busca_informacoes_gerais TABLES lt_inf_gerais.
  ...
  CONSTANTS: BEGIN OF lc_inf_gerais,
