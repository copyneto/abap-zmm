  ...
  as select from ZI_MM_INVENTARIO_H_TOTAL_SUM
{
  ...
  key Documentid,
      ...
      @Semantics.amount.currencyCode : 'Currency'
      PriceStock,
      case when PriceStock is not initial and PriceDiff is not initial
           then fltp_to_dec( 
                ( cast( 1 as abap.fltp ) - ( cast( PriceDiff as abap.fltp ) / cast( PriceStock as abap.fltp ) ) ) * cast( 100 as abap.fltp ) 
                as abap.dec(11,2) ) 
           else cast( 0 as abap.dec(11,2) )
           end as Accuracy,
