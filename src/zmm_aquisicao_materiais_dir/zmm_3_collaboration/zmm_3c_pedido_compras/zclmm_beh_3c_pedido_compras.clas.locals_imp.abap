...
CLASS lcl_job IMPLEMENTATION.
  ...
  METHOD objeto_subobjeto.
    READ ENTITIES OF zi_mm_3c_pedido_compras IN LOCAL MODE
      ENTITY job
      ALL FIELDS WITH CORRESPONDING #( keys )
      RESULT DATA(lt_job).

    MODIFY ENTITIES OF zi_mm_3c_pedido_compras IN LOCAL MODE
      ENTITY job
  ...
  METHOD execute.
    ...
    READ ENTITIES OF zi_mm_3c_pedido_compras
...
*           high   = COND #( WHEN strlen( lv_high ) EQ 8 THEN |{ lv_high+4(4) }{ lv_high+2(2) }{ lv_high(2) }| ) ) ).
    ...
    DATA: lr_eindt TYPE RANGE OF eket-eindt,
          lr_werks TYPE RANGE OF ekpo-werks,
          lr_matnr TYPE RANGE OF ekpo-matnr.

    LOOP AT lt_variant ASSIGNING FIELD-SYMBOL(<fs_variant>).
      CONDENSE: <fs_variant>-low  NO-GAPS,
                <fs_variant>-high NO-GAPS.

      CASE <fs_variant>-dataelement.
        WHEN 'S_EINDT'.
          APPEND VALUE #( sign   = <fs_variant>-sign
                          option = <fs_variant>-opti
                          low    = COND #( WHEN strlen( <fs_variant>-low ) EQ 8
                                           THEN |{ <fs_variant>-low+4(4) }{ <fs_variant>-low+2(2) }{ <fs_variant>-low(2) }| )
                          high   = COND #( WHEN strlen( <fs_variant>-high ) EQ 8
                                           THEN |{ <fs_variant>-high+4(4) }{ <fs_variant>-high+2(2) }{ <fs_variant>-high(2) }| )
                        ) TO lr_eindt.
        WHEN 'S_WERKS'.
          APPEND VALUE #( sign   = <fs_variant>-sign
                          option = <fs_variant>-opti
                          low    = <fs_variant>-low
                          high   = <fs_variant>-high ) TO lr_werks.

        WHEN 'S_MATNR'.
          APPEND VALUE #( sign   = <fs_variant>-sign
                          option = <fs_variant>-opti
                          low    = CONV matnr18( |{ <fs_variant>-low  ALPHA = IN }| )
                          high   = CONV matnr18( |{ <fs_variant>-high ALPHA = IN }| )
                        ) TO lr_matnr.
    ...
    "Chama o programa
    SUBMIT zmmr_3c_pedido_compras
      WITH s_eindt IN lr_eindt
