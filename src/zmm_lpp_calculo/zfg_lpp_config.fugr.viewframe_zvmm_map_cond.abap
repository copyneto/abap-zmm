*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVMM_MAP_COND
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVMM_MAP_COND       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVMM_MAP_COND'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVMM_MAP_COND_EXTRACT
TOTAL = ZVMM_MAP_COND_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVMM_MAP_COND'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVMM_MAP_COND-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVMM_MAP_COND_EXTRACT
TOTAL = ZVMM_MAP_COND_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVMM_MAP_COND-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVMM_MAP_COND'
...
TOTAL = ZVMM_MAP_COND_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVMM_MAP_COND-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVMM_MAP_COND-UPD_FLAG NE SPACE.
